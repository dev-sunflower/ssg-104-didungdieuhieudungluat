...
const FrogModel = forwardRef<THREE.Group, Props>(
  ({ interactive = false, onClick }, forwardedRef) => {
    ...
    useEffect(() => {
      ...
      const handleWheel = (e: WheelEvent) => {
        ...
        targetZoom.current = Math.max(
          0.5,
          Math.min(2.5, targetZoom.current - e.deltaY * 0.001),
        );
      };
      ...
      const handlePointerMoveGlobal = (e: PointerEvent) => {
        if (isDragging.current) {
          ...
          targetRotation.current.y = Math.max(
            -Math.PI / 4,
            Math.min(Math.PI / 4, targetRotation.current.y),
          );
