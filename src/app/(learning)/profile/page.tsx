...
export default async function ProfilePage() {
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      {/* ── 2. Thống kê (Stats Strip) ───────────────────── */}
      <section>
        <h2 className="heading-feature text-text-primary mb-4">📊 Tổng quan học tập</h2>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <Card className="border-border bg-bg-card shadow-whisper p-5">
            ...
            <p className="text-3xl font-serif text-brand">
              {passedExams}
            </p>
          </Card>
          <Card className="border-border bg-bg-card shadow-whisper p-5">
      ...
      {/* ── 3. Quản lý thông tin & Lịch sử thi (2 cột) ──── */}
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        
        {/* Lịch sử thi */}
        <div className="lg:col-span-7 flex flex-col gap-4">
          <h2 className="heading-feature text-text-primary mb-2">🏷️ Lịch sử đề thi</h2>
              ...
              exams.slice(0, 5).map((exam) => {
                ...
                return (
                  <Card key={exam.id} className="border-border bg-bg-card shadow-sm p-4 flex flex-row items-center gap-4">
                    ...
                    <Link 
                      href={`/exam`}
                      className="shrink-0 text-text-tertiary hover:text-brand text-xl px-2 transition-colors"
                      title="Thi lại"
                    >
                      ↻
                    </Link>
        ...
        <Card className="lg:col-span-5 border-border bg-bg-card shadow-whisper p-6 sticky top-24">
          <h2 className="heading-feature text-text-primary mb-6">⚙️ Cài đặt tài khoản</h2>
