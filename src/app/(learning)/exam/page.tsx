...
import { LuPartyPopper, LuFrown, LuTriangleAlert, LuRotateCcw, LuClipboard, LuTarget, LuTimer } from 'react-icons/lu'
...
function ExamContent() {
  ...
  useEffect(() => {
    supabase
      ...
      .eq('code', 'A1')
      .single()
      .then(({ data }) => { if (data) setA1Type(data) })
  }, [supabase])
    ...
    return (
      <div className="flex flex-col gap-5">
        <div className="rounded-3xl border border-[#1E1E1E]/10 bg-[linear-gradient(145deg,#FFF4D6_0%,#FFE8A8_100%)] p-5 md:p-6">
          <h1 className="text-2xl font-extrabold text-[#1E1E1E] md:text-3xl" style={{ fontFamily: 'var(--font-caveat)' }}>
            📝 Thi thử sát hạch
          </h1>
        </div>
          ...
          <Card.Content className="p-6 text-center">
            ...
            <div className="mt-4 flex items-center justify-center gap-6">
              <div className="text-center">
                <div className="text-4xl text-text-primary" style={{ fontFamily: 'var(--font-caveat)' }}>{correct}/{questions.length}</div>
                <div className="mt-0.5 text-xs text-text-tertiary">Số câu đúng</div>
              </div>
              <div className="h-12 w-px bg-[#1E1E1E]/10" />
              <div className="text-center">
                <div className={`text-4xl ${pct >= 80 ? 'text-[#4ECDC4]' : pct >= 60 ? 'text-[#F4A616]' : 'text-crimson'}`} style={{ fontFamily: 'var(--font-caveat)' }}>{pct}%</div>
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between md:hidden">
        ...
        <div className={`flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-sm font-bold ${isLowTime ? 'border-red-200 bg-red-50 text-red-500' : 'border-[#F4A616]/30 bg-[#FFF4D6] text-[#1E1E1E]'}`} style={{ fontFamily: 'var(--font-caveat)' }}>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-[280px_1fr]">
        ...
        <div className="hidden md:flex flex-col gap-4">
          {/* Timer */}
          <div className={`rounded-2xl border p-4 text-center ${isLowTime ? 'border-red-200 bg-red-50' : 'border-[#F4A616]/30 bg-[#FFF4D6]'}`}>
            <p className="mb-1 text-xs font-medium text-[#1E1E1E]/50 uppercase tracking-wider">Thời gian còn lại</p>
            <p className={`text-5xl font-bold ${isLowTime ? 'text-red-500' : 'text-[#1E1E1E]'}`} style={{ fontFamily: 'var(--font-caveat)' }}>
              {formatTime(timeLeft)}
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-[#1E1E1E]/10 bg-white p-3">
            <div className="mb-2 flex items-center justify-between">
              <p className="text-xs font-medium text-[#1E1E1E]/50">{answered}/{questions.length} đã trả lời</p>
              {questions.some((q) => q.is_critical) && (
                <span className="flex items-center gap-1 text-[10px] text-crimson/70">
                  <span className="h-1.5 w-1.5 rounded-full bg-crimson inline-block" /> Điểm liệt
                </span>
              )}
            </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex gap-3">
            ...
            {currentIndex < questions.length - 1 ? (
              ...
              <Button fullWidth onPress={handleSubmit} className="rounded-2xl bg-[#F4A616] py-3 font-medium text-[#1E1E1E] hover:bg-[#e59b11] md:hidden">
                Nộp bài
              </Button>
            )}
          </div>
